					...
					if (enemiesAliveInLane(laneRequested) && (fireImmediately || enemiesAliveInLane(playerData.CurrentLane)))
					{
						...
						if (itemAbilitiesOn)
						{
							...
							if (fireImmediately || laneRequested == playerData.CurrentLane) // Really sure to work on the current lane
							{
								...
								if (random.Next(3) == 0 && hasPurchasedAbility(Abilities.ReflectDamage) && !isAbilityCoolingDown(Abilities.ReflectDamage))
								{
								...
								if (countLiveMonstersOnLane(laneRequested) >= 3 || (bossMonsterOnLane(laneRequested) && farmingGold))
								{
									if (highestHpFactorOnLane(laneRequested) > 0.75m)
									{
										if (random.Next(3) == 0 && hasPurchasedAbility(Abilities.StealHealth) && !isAbilityCoolingDown(Abilities.StealHealth))
										{
											if (abilities) abilties_json += ",";
										...
										{
											if (random.Next(3) == 0 && hasPurchasedAbility(Abilities.Revive) && !isAbilityCoolingDown(Abilities.Revive)) // Resurrection
											{
												if (abilities) abilties_json += ",";
